...
$.widget( "ui.autocomplete", {
		...
		this.element
			...
			.bind( "blur.autocomplete", function( event ) {
				...
				self.closing = setTimeout(function() {
					var button = $("#youtube_button").data("clicked");
					console.log(button);
					if (!button == 1) {
						self.close( event );
					}
				...
				if ( !$( event.target ).closest( ".ui-menu-item" ).length ) {
					setTimeout(function() {
						$( document ).one( 'mousedown', function( event ) {;
							// var test = (event.target == $("#youtube_button"))
	...
	//this._trigger("autocompletesearchcomplete", [content]);
  
		if ( !this.options.disabled && content && content.length ) {
			this._suggest( content );
			this._trigger( "open" );
		} else {
			//Disabled this and its working good
			//this.close();
		}
		this.pending--;
		if ( !this.pending ) {
